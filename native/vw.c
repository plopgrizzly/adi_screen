...
vw_t vw_vulkan(const char* title) {
	vw_t vulkan;
	...
	const char *extensions[] = { "VK_KHR_surface",
		...
		"VK_KHR_xcb_surface",
	#endif
// TODO: better way to change to and from validation layers
//		"VK_EXT_debug_report",
	};
	const VkApplicationInfo applicationInfo = {
		...
		.pEngineName = "Aldaron's Device Interface, ADI.",
		.engineVersion = 1,
...
//		.ppEnabledLayerNames = &layernames,
	...
	PFN_vkCreateInstance vk_create_instance = (PFN_vkCreateInstance)
		vkGetInstanceProcAddr(NULL, "vkCreateInstance");
	printf("Got address\n");
	vw_vulkan_error("Failed to create vulkan instance.",
		vk_create_instance(&instance_ci, NULL, &vulkan.instance));
	return vulkan;
}
...
//		vkGetInstanceProcAddr(NULL, "vkEnumeratePhysicalDevices");
...
static inline void vw_vulkan_device(vw_t* vulkan) {
...
//		.ppEnabledLayerNames = NULL,
		...
		.pEnabledFeatures = NULL, //&features,
	};
	vw_vulkan_error("Failed to create logical device!", vkCreateDevice(
		vulkan->gpu, &device_ci, NULL, &vulkan->device));
}

static inline void vw_vulkan_queue(vw_t* vulkan) {
	vkGetDeviceQueue(vulkan->device, vulkan->present_queue_index, 0,
		&vulkan->present_queue);
}

static inline void vw_vulkan_command_buffer(vw_t* vulkan) {
	VkCommandPoolCreateInfo command_pool_ci = {
		.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO,
		.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT,
		.queueFamilyIndex = vulkan->present_queue_index,
	};
	vw_vulkan_error("Failed to create command pool.", vkCreateCommandPool(
		vulkan->device, &command_pool_ci, NULL, &vulkan->cmd_pool));
	...
	vw_vulkan_error("Failed to allocate draw command buffer.",
...
//	vw_t wrapper = vw_vulkan(title); // Initialize Vulkan
	...
	vw_vulkan_command_buffer(&wrapper); // Link CMD Buffer to Vulkan
	puts("Resizing");
	vw_vulkan_resize(&wrapper); // Resize
	puts("ResiZed");
	return wrapper;
}
